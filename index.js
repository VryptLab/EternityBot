...
const centerBanner = (text = 'EternityBot') => {
  const output = cfonts.render(text, { font: 'tiny', align: 'left', colors: ['yellow'], background: 'transparent', letterSpacing: 1, lineHeight: 1, space: true });
...
const fetchGroupMetadata = async (conn, groupId) => {
  ...
  try {
    ...
    if (!isPrompting) LOG.error(`Failed to fetch group metadata for ${groupId}: ${err?.message ?? err}`);
    return null;
  }
};
...
async function startWA() {
  ...
  conn.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
    ...
    if (connection === 'open') {
      ...
      reconnectAttempts = 0;
      try { await conn.insertAllGroup(); } catch (err) { safeLog.error(`Failed to insert groups: ${err?.message ?? err}`); }
    }
  });
